...
\[
\text{Price\_Change\_Percent} = \frac{\text{close} - \text{Price\_Lag1}}{\text{Price\_Lag1}} \times 100
\]
...
The model is trained using the `xgboost` package in R. The hyperparameters are configured as follows:
- **Number of boosting rounds**: 100
- **Learning rate ($\eta$)**: 0.1
- **Maximum tree depth**: 6
- **Column sampling rate**: 80%
- **Row sampling rate**: 80%
- **Objective function**: Squared error regression ($\text{reg:squarederror}$)

### Training and Testing

The training dataset includes observations up to December 31, 2024, while the testing dataset covers the period from January 1, 2025, to March 31, 2025. The predictors include:
\[
\{\text{Price\_Lag1}, \text{Price\_Change\_Percent}, \text{volume}, \text{open}, \text{high}, \text{low}, \text{adjusted}\}
\]
The outcome variable, $y_i$, is defined as:
\[
y_i = \text{Price\_Diff} = \text{close}_i - \text{close}_{i-1}
\]


We run the model in R [@citeR] using the `xgboost` package of @xgboost. 
...
# Filter data for the prediction period (e.g., Q1 2025)
prediction_data <- price_analysis_data %>%
...
X_pred <- model.matrix(~ . - 1, data = prediction_data[, predictors])

#### Generate Predictions ####
# Predict daily price differences
predicted_diffs <- predict(xgb_model, X_pred)

# Add predictions to the prediction dataset
prediction_data <- prediction_data %>%
  mutate(
    Predicted_Diff = predicted_diffs,
    Predicted_Close = Price_Lag1 + Predicted_Diff # Calculate predicted closing price
  )

#### Extract Predictions for Each Stock ####
predicted_stock_prices <- prediction_data %>%
  select(symbol, date, Price_Lag1, Predicted_Diff, Predicted_Close) %>%
  arrange(symbol, date)
